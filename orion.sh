...
useradd -m -s /bin/bash callhome 2>/dev/null
...
existing_uuid="$(grep -hPo '/opt/socketcallhome/socketcallhome/\K([[:xdigit:]]|-)*' /home/callhome/callhome*.sh | head -n 1)"
if [ -z "$existing_uuid" ]; then
   my_uuid="$(cat /proc/sys/kernel/random/uuid)";
else
    my_uuid="$existing_uuid"
fi
my_password="fake placeholder" #my_password="$(cat /proc/sys/kernel/random/uuid)"
my_pubkey="$(cat /home/callhome/.ssh/id_ed25519.pub)"
my_privkey="fake placeholder" #my_privkey="$(cat /home/securin/.ssh/id_ed25519)"
...
systemctl enable securincallhome-orion.service 2>/dev/null
systemctl restart securincallhome-orion.service 2>/dev/null
...
echo "$package" > /root/securinsetup-orion.txt
...
echo "$package"
echo
echo "This value has also been stored at /root/securinsetup-orion.txt"
